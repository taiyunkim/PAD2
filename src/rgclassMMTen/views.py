...
from .models import Signal_mmTen_db_info
...
#             'selected_field': """368.Wdr5.CCE
    ...
    # table = Signal_mmTen_db.objects.all().values('fileID', 'filename')
    ...
    if form.is_valid():
        ...
        return redirect('/PADmm10/result')
...
def rgClassifyResult(request):
    ...
    path = os.path.join(settings.MEDIA_ROOT, 'signal_mmTen_file_db', region)
    ...
    signal_coverage_value = Signal_mmTen_db_info.objects.filter(fileID__in = f_order, region = region).values_list('fileID', 'value')
